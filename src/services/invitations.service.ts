...
export class InvitationsService {
	...
	async addInvitees(
		...
		// Add invitees
		await this.invitationsRepo.addInvitees(bookingId, validUserIds);

		// Send email notifications to all invited users (async, don't block)
		...
		// Return the updated list of invitees
	...
	async respondToInvitation(
		...
		// Update the status
		await this.invitationsRepo.updateStatus(bookingId, userId, status);

		// If accepted, send notification email to booking owner
		if (status === 'accepted') {
			const invitee = await this.usersRepo.findById(userId);
